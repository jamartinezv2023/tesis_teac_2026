...
En la sociedad del conocimiento, una proporción significativa de las oportunidades educativas se materializa mediante entornos digitales (LMS, plataformas, recursos interactivos, repositorios, videoconferencias). Por tanto, la inclusión requiere que estos entornos sean accesibles, usables y comprensibles para personas con diversos perfiles funcionales y contextos de uso.
...
\item \textbf{Comprensibilidad:} lenguaje claro, instrucciones explícitas, retroalimentación oportuna, reducción de carga cognitiva.
...
La accesibilidad no es un módulo aislado: constituye una condición transversal del diseño tecnológico, del modelo de datos (p.\ ej., registro de preferencias de acceso) y de la visualización de resultados diagnósticos (p.\ ej., reportes legibles, exportables y configurables).
...
\begin{tikzpicture}[
  ...
  arrow/.style={-{Stealth[length=2.6mm]}, line width=0.7pt, rounded corners=2pt, shorten <=2pt, shorten >=2pt},
  frame/.style={draw, rounded corners=2mm, dashed, inner sep=8pt}
]

% 1) Núcleo teórico (pilares)
...
\emph{Prototipo investigativo basado en datos: arquitectura modular + analítica/\IA + accesibilidad/usabilidad + gobernanza ética}};
...
% Realimentación (DBR): m6 -> m3 por el borde
...
% 6) Ética y accesibilidad (transversal)
...
% Ética -> sistema por el lado derecho
\coordinate (eRight) at ($(eth.east)+(14mm,0)$);
\coordinate (sRight) at ($(sys.east)+(14mm,0)$);
\draw[arrow] (eth.east) -- (eRight) |- (sRight) -- (sys.east);

% 7) Marco DBR
\node[frame, fit=(sys) (m1) (m2) (m3) (m4) (m5) (m6),
      label={[align=center]above:\textbf{Ciclo iterativo de diseño, implementación y mejora (Design-Based Research -- DBR)}}] (dbrframe) {};
